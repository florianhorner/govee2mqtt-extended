...
* **Native fan entities** — Fans and air purifiers appear as Home Assistant `fan` entities with power control, plus a speed slider, preset modes, and oscillation on models whose metadata reports them, so `fan.set_percentage` and `fan.set_preset_mode` work directly (this fork only).
